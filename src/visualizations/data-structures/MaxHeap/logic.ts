...
export function generateMaxHeapSteps(): VisualizationStep[] {
  const steps: VisualizationStep[] = [];
  let stepId = 0;
  const heap: number[] = [];
  const insertValues = [10, 30, 20, 40, 35, 25, 50];

  // --- Initial state ---
  steps.push({
    id: stepId++,
    description: "Starting Max-Heap visualization. We will insert values one by one and then extract the maximum twice.",
    action: "highlight",
    highlights: [],
    data: {
      nodes: [],
  ...
  steps.push({
    id: stepId++,
    description: `All ${insertValues.length} values inserted. Max-Heap root = ${heap[0]} (maximum). Array: [${heap.join(", ")}]`,
    action: "complete",
    highlights: [],
    data: {
  ...
  for (let extraction = 1; extraction <= 2; extraction++) {
    ...
    steps.push({
      id: stepId++,
      description: `Extract-Max #${extraction}: Remove root ${maxVal} (maximum). Swap with last element ${lastVal}`,
      action: "remove",
      highlights: [
    ...
    if (heap.length === 0) {
      steps.push({
        id: stepId++,
        description: `Extracted ${maxVal}. Heap is now empty.`,
        action: "complete",
        highlights: [],
        data: {
          nodes: [],
    ...
    while (true) {
      ...
      if (largest !== current) {
        const swapHl = new Map<number, "active" | "comparing" | "swapping" | "completed" | "selected">();
        swapHl.set(current, "swapping");
        swapHl.set(largest, "swapping");

        [heap[current], heap[largest]] = [heap[largest], heap[current]];

        const swapTree = buildTreeFromArray(heap, largest, swapHl);
        steps.push({
          id: stepId++,
          description: `Swap ${heap[largest]} and ${heap[current]}: larger child ${heap[current]} moves up`,
          action: "swap",
          highlights: [{ indices: [current, largest], color: "swapping" }],
          data: {
            nodes: swapTree.nodes,
            edges: swapTree.edges,
            rootId: "0",
            currentNodeId: String(largest),
            operation: "swap down",
            auxiliaryArray: [...heap],
            auxiliaryHighlights: buildArrayHighlights(swapHl),
          } satisfies TreeStepData,
        ...
        steps.push({
          id: stepId++,
          description: `${heap[current]} is larger than both children — heap property restored. Extracted ${maxVal}`,
          action: "complete",
          highlights: [{ indices: [current], color: "completed" }],
          data: {
            nodes: buildTreeFromArray(heap, null, new Map()).nodes,
  ...
  steps.push({
    id: stepId++,
    description: `Max-Heap visualization complete! Final heap: [${heap.join(", ")}] with root = ${heap[0]}`,
    action: "complete",
